...
jest.mock(
    "../../src/logout/utils/deleteCookies",
    () => ({
...
const deleteCookies =
    utils.deleteCookies as jest.Mock;
...
describe("controller.ts", () => {
  ...
  it("should redirect the user to One Login to log out", async () => {
    const req = getMockReq({
      ...
      cookies: {id_token: "id_token", access_token: "access_token", state: "state", app: "govuk-staging"}
    });
    const { res } = getMockRes({ cookies: {id_token: "id_token", access_token: "access_token", state: "state", app: "govuk-staging"}});

    await logoutGetController(req, res);

    expect(res.redirect).toHaveBeenCalled();
    expect(req.oidc.endSessionUrl).toHaveBeenCalled();
    expect(deleteCookies).toHaveBeenCalledWith(req, res, ["id_token", "access_token", "app"])
  });
});

...
describe("controller.ts", () => {
    it("should return 200 and a bearer access token in the response body", async () => {
        ...
        const req = getMockReq({
            body: { grant_type: "mock_grant_type", "pre-authorized_code": "mock_pre-authorized_code" },
        });

        const validateGrantType = validateTokenRequest.validateGrantType as jest.Mock;
        validateGrantType.mockReturnValueOnce(true);

        const getPreAuthorizedCodePayload = validateTokenRequest.getPreAuthorizedCodePayload as jest.Mock;
        ...
        createAccessToken.mockReturnValueOnce("eyJ0eXAiOiJKV1Qh.eyJzdWIiOiM.9nQevZ--Asqx5ltCWvw_AvVNDA");

...
import {
    S3Client,
    PutObjectCommand,
    GetObjectCommand, GetObjectCommandOutput, NoSuchKey,
...
describe("s3Service.ts", () => {
    ...
    it("should save a photo to the s3 bucket", async () => {
        ...
        expect(s3Mock).toHaveReceivedCommandWith(PutObjectCommand, {Bucket: bucketName, Key: imageName, Body: imageBuffer,});
    })
    ...
    describe("Get Photo", () => {
        ...
        it("should get a photo from the s3 bucket", async () => {
            ...
            const s3Mock = mockClient(S3Client);
            s3Mock.on(GetObjectCommand).resolvesOnce({Body: mockS3Response("base64image")
        ...
        it("it should throw the error thrown by the S3 client", async () => {
            ...
            await expect(
                getPhoto(imageName, bucketName)
            ).rejects.toThrow("Error getting image");
            expect(s3Mock).toHaveReceivedCommandWith(GetObjectCommand, {Bucket: bucketName, Key: imageName});

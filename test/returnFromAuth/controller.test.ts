...
describe("controller.ts", () => {
    ...
    it("should return 500 when the auth server responds with an error", async () => {
        ...
        expect(loggerErrorSpy).toHaveBeenNthCalledWith(1, "access_denied - some description");
    ...
    it("should return 500 when the token response does not have an access token", async () => {
        ...
        expect(loggerErrorSpy).toHaveBeenNthCalledWith(1, "No access token received");
        expect(res.status).toHaveBeenCalledWith( 500)
    });
    ...
    it("should return 200 and redirect to /select-document", async () => {
        ...
        expect(res.cookie).toHaveBeenNthCalledWith( 1, "access_token", "access_token", {"httpOnly": true, "maxAge": 100000})
        expect(res.cookie).toHaveBeenNthCalledWith( 2, "id_token", "id_token", {"httpOnly": true, "maxAge": 100000})
